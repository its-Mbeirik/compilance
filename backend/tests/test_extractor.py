...
# ---------------------------------------------------------------------------
# Données synthétiques — Code du Travail Mauritanien
# ---------------------------------------------------------------------------
...
def test_no_duplicate_clause_types():
    clauses = _generate_clauses(CDD_EXTRACTION, "mauritania_labor")
    types = [c["type_clause"] for c in clauses]
...
def test_llm_clauses_preserved():
    ...
    with_llm_clauses = dict(CDD_EXTRACTION)
    with_llm_clauses["clauses"] = [{
        "clause_id": "abc12345",
        "type_clause": ClauseType.PERIODE_ESSAI.value,
        "text": "La période d'essai est fixée à 2 mois conformément au Code du Travail.",
        "jurisdiction_hint": "mauritania_labor",
    }]
    clauses = _generate_clauses(with_llm_clauses, "mauritania_labor")
    essai_clauses = [c for c in clauses if c["type_clause"] == ClauseType.PERIODE_ESSAI.value]
    assert len(essai_clauses) == 1, "La clause période d'essai ne doit apparaître qu'une fois"
...
def test_extractor_node_real_llm_labor():
    """Test d'intégration : extraction réelle sur un CDD mauritanien."""
    import os
    ...
    assert not result.get("errors"), f"Erreurs inattendues: {result.get('errors')}"
